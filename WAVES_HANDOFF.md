...
cd dashboard && npm run dev          # dashboard :3000  ->  /waves
```
...
src/seismo/api/{app,models}.py     GET /waves, GET /waves/{id}
dashboard/app/waves/               index + detail (timeline, not a node-link graph)
dashboard/components/VerdictChip.tsx
...
```bash
uv run pytest tests/test_waves.py -q                          # 21 passed
uv run pytest -q -m "not hindcast and not llm_local"          # 281 passed
...
⚠️ `ruff format` reformats ~22 files unrelated to this work — the repo carries pre-existing format
drift, so CI's `ruff format --check` step is probably already red on `main`. It was deliberately left
alone rather than folded into this branch. Worth a separate formatting-only commit.
