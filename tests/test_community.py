...
def test_claude_cli_parses_fenced_json_and_pins_the_model(monkeypatch) -> None:
    """The CLI has no forced tool call, so the reply is prompted for and parsed. Models routinely
    wrap it in a ```json fence."""
    import subprocess

    from seismo.checkpoints.llm import _claude_cli
    ...
    def _fake_run(cmd, **kw):  # noqa: ANN001
        ...
        return subprocess.CompletedProcess(
            cmd, 0, stdout=_cli_envelope('```json\n{"overall_sentiment": "mixed"}\n```'), stderr=""
        )

    monkeypatch.setattr(subprocess, "run", _fake_run)
    out = _claude_cli("sys", "user", {"type": "object"}, model="claude-haiku-4-5")

    assert out.content == {"overall_sentiment": "mixed"}
    assert out.model == "claude_cli:claude-haiku-4-5"
    assert out.cost_usd == Decimal("0.05")  # feeds llm_budget_usd
    # Pinned to the configured model, not whatever an interactive session is using.
    assert seen["cmd"][seen["cmd"].index("--model") + 1] == "claude-haiku-4-5"
    assert "--strict-mcp-config" in seen["cmd"]
    # Neutral cwd, or every call would also carry the repo's CLAUDE.md.
    assert seen["cwd"] and "OportunityRadar" not in seen["cwd"]


def test_claude_cli_reports_what_it_actually_said_on_non_json(monkeypatch) -> None:
    import subprocess
    ...
    def _fake_run(cmd, **kw):  # noqa: ANN001
        return subprocess.CompletedProcess(
            cmd,
            0,
            stdout=_cli_envelope("Could you provide the comments from these issues?"),
            stderr="",
        )

    monkeypatch.setattr(subprocess, "run", _fake_run)
    with pytest.raises(RuntimeError, match="Could you provide the comments"):
        _claude_cli("sys", "user", {"type": "object"})
